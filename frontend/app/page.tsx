...
type Farm = {
  ...
  checklist?: {
    ...
    dueDate?: string | null;
    renewalComments?: string | null;
    renewalDate?: string | null;
    documentStatus?: string | null;
  }[];
  documents?: { originalName: string; checklistDocumentKey?: string | null }[];
};
type Obligation = {
  id: string;
  title: string;
...
const pageMeta: Record<Tab, { title: string; subtitle: string }> = {
  "agente-ia": {
    title: "Agente IA",
    subtitle: "Converse com seu assistente agronômico",
  },
  fazendas: {
    title: "Fazendas",
...
function FarmDatePicker({
  ...
  const [month, setMonth] = useState(
    validDate?.getMonth() ?? new Date().getMonth(),
  );
  ...
  function chooseDate(nextDay = day, nextMonth = month, nextYear = year) {
    ...
    setYear(nextYear);
    onChange(
      `${nextYear}-${String(nextMonth + 1).padStart(2, "0")}-${String(safeDay).padStart(2, "0")}`,
    );
  }
  ...
  return (
    <div className="farm-date-picker">
      <div className="farm-date-field">
        <button
          ref={triggerRef}
          type="button"
          className={`farm-information-input farm-date-trigger ${noDate ? "indeterminate" : ""}`}
          onClick={openPicker}
          disabled={disabled}
          aria-expanded={open}
        >
      ...
      {open && (
        <div
          className="farm-date-popover"
          style={{ top: popoverPosition.top, left: popoverPosition.left }}
        >
          <div className="farm-date-columns">
            <div className="farm-date-column">
              <span>Dia</span>
              <div>
            ...
            <div className="farm-date-column farm-date-month-column">
              <span>Mês</span>
              <div>
              ...
              <div>
                {years.map((item) => (
                  <button
                    ...
                    key={item}
                    onClick={() => chooseDate(day, month, item)}
                  >
                    {item}
                  </button>
                ))}
          ...
          <span className="farm-date-no-date">
            <input
              type="checkbox"
              checked={noDate}
              disabled={disabled}
          ...
          <div className="farm-date-actions">
            <button
              type="button"
...
function coverFor(farm: Farm) {
  return farm.coverImage?.startsWith("data:image/") ||
    farm.coverImage?.startsWith("/")
    ? farm.coverImage
    : farm.coverImage?.startsWith("fazenda_")
      ? `/${farm.coverImage}`
      : "/fazenda_sol_nascente.png";
}

export default function Home({
  initialTab = "agente-ia",
}: {
  initialTab?: Tab;
}) {
  ...
  async function login(event: FormEvent) {
    ...
    try {
      ...
      window.localStorage.setItem("agroclaw_token", data.accessToken);
    ...
    <main className="login-page">
      <section className="login-card">
        <div className="login-brand">
          ...
          <div>
            <strong>AgroClaw</strong>
            <span>Farm Manager</span>
          </div>
        </div>
        <h1>Acessar sistema</h1>
        <p>Entre com o usuário administrador para continuar.</p>
      ...
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        <div className="sidebar-brand">
          ...
          <div className="brand-text">
            <span className="brand-name">AgroClaw</span>
            <span className="brand-tagline">Farm Manager</span>
          </div>
        </div>
...
function AiPage({ token }: { token: string }) {
  const [messages, setMessages] = useState<Message[]>([
    {
      sender: "ai",
      content:
        "Olá! Sou seu assistente de inteligência artificial para gerenciamento de Fazendas.\n\nComo posso ajudar você hoje?",
      time: "agora",
    },
  ]);
  ...
  async function send(content = input) {
    ...
    const text = content.trim() || `📎 ${attachment?.name}`;
    const prompt = attachmentText
    ...
    setMessages((old) => [
      ...old,
      { sender: "user", content: text, time: "agora" },
    ]);
    setLoading(true);
    setError("");
    try {
      const r = await fetch(`${API_URL}/ai/messages`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ content: prompt, model, attachmentText }),
      });
      ...
      setMessages((old) => [
        ...old,
        { sender: "ai", content: data.content, time: "agora" },
      ]);
    } catch (e) {
  ...
  return (
    <div id="tab-agente-ia" className="tab-page active">
      <div className="chat-header-bar">
        ...
        <button
          ...
          onClick={async () => {
            await fetch(`${API_URL}/ai/messages`, {
              method: "DELETE",
              headers: { Authorization: `Bearer ${token}` },
            });
            setMessages([]);
      ...
      <div className="chat-container">
              ...
              <div className="message-content">
                <div className="message-header">
                  <span className="message-sender">
                    {message.sender === "ai" ? "AgroClaw IA" : "Você"}
                  </span>
        ...
        <div className="chat-input-area">
          <div className="chat-input-wrapper">
            <div className="chat-input-row">
              <textarea
                ...
                placeholder="Pergunte algo sobre suas fazendas..."
                rows={1}
              />
                  ...
                  onChange={(event) => {
                    ...
                    if (
                      ...
                      reader.onload = (loaded) =>
                        setAttachmentText(String(loaded.target?.result ?? ""));
                      reader.readAsText(file);
                    } else setAttachmentText("");
                  }}
                />
                <button
                  ...
                  onClick={() =>
                    document.getElementById("ai-attachment")?.click()
                  }
                >
                  <Paperclip size={18} />
                </button>
          ...
          <div className="chat-disclaimer">
            AgroClaw IA pode gerar informações imprecisas. Verifique sempre
            dados críticos.
          </div>
        </div>
      </div>
...
function FarmsPage({ token }: { token: string }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/farms`, { headers: { Authorization: `Bearer ${token}` } })
      .then(async (r) => {
        if (!r.ok) throw new Error("Não foi possível carregar as fazendas.");
        return r.json();
      })
      .then(setFarms)
  ...
  const hectares = useMemo(
    () => farms.reduce((sum, farm) => sum + Number(farm.areaHectares), 0),
    [farms],
  );
  async function deleteFarm(farm: Farm) {
    if (!window.confirm(`Deseja excluir a fazenda "${farm.name}"?`)) return;
    const response = await fetch(`${API_URL}/farms/${farm.id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    if (response.ok)
      setFarms((old) => old.filter((item) => item.id !== farm.id));
    else setError("Não foi possível excluir a fazenda.");
  }
  return (
    <div id="tab-fazendas" className="tab-page active">
      <div className="fazendas-header">
        <div>
          <h2>Suas Fazendas</h2>
          <button
            className="btn btn-primary"
        ...
        <div className="fazendas-stats">
          <div className="stat-item">
            <span className="stat-value">{farms.length}</span>
            <span className="stat-label">Fazendas</span>
          </div>
          <div className="stat-item">
      ...
      <div className="fazendas-grid">
        {farms.map((farm) => (
          <div className="fazenda-card" key={farm.id}>
            <div className="fazenda-cover">
              <Image
                src={coverFor(farm)}
                alt={farm.name}
                width={640}
            ...
            <div className="fazenda-body">
              <h3 className="fazenda-name">{farm.name}</h3>
              <div className="fazenda-location">
                <Search size={12} /> {farm.location}
              </div>
              <div className="fazenda-metrics">
                <div className="metric">
                  <div className="metric-value">{farm.areaHectares}ha</div>
                  <div className="metric-label">Área</div>
                </div>
                <div className="metric">
                  <div className="metric-value">{farm.culture}</div>
                  <div className="metric-label">Cultura</div>
                </div>
                <div className="metric">
                  <div className="metric-value">
                    {farm.checklist?.length ?? 0}
                  </div>
                  <div className="metric-label">Documentos</div>
                </div>
              </div>
              <div className="fazenda-actions">
                <button
                  className="btn btn-primary"
      ...
      {wizardOpen && (
        <FarmWizard
          ...
          onCreated={async () => {
            ...
            if (response.ok) setFarms(await response.json());
          }}
        />
      )}
      {editingFarm && (
...
function FarmDetailsModal({
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/folders`, {
      ...
      .then((data) =>
        setDocumentFolders(
          data.filter(
            (folder: { farmId?: string | null; name: string }) =>
              folder.farmId === farm.id,
          ),
        ),
      )
      .catch(() => undefined);
  ...
  async function updateChecklistField(
    ...
    const previous = checklistUpdateQueues.current[itemId] ?? Promise.resolve();
    const update = previous
      .catch(() => undefined)
      .then(async () => {
        const response = await fetch(
          `${API_URL}/farms/${farm.id}/checklist/${itemId}`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  return (
    ...
    >
      <div className="modal-card farm-overview-modal">
            ...
            <div>
              ...
              <div className="modal-meta-list">
                <div className="modal-meta-item">
                  ...
                  <span className="meta-value">{farm.location}</span>
                </div>
                <div className="modal-meta-item">
                  <span className="meta-label">Área Total</span>
                  <span className="meta-value">{farm.areaHectares} ha</span>
                </div>
                <div className="modal-meta-item">
                  <span className="meta-label">Cultura Ativa</span>
                  <span className="meta-value">{farm.culture}</span>
                </div>
                <div className="modal-meta-item">
                  <span className="meta-label">Status</span>
                  <span className="meta-value">{farm.status}</span>
                </div>
              </div>
            </div>
            <div>
              <h3>Panorama da Fazenda</h3>
              <p className="modal-docs-intro">
                Selecione uma área para continuar.
              </p>
              <div className="farm-overview-blocks">
                <button
                  className={`farm-overview-block ${activeBlock === "documentos" ? "active" : ""}`}
                  onClick={() => {
                    setActiveBlock("documentos");
                ...
                <button
                  className={`farm-overview-block ${activeBlock === "maquinas" ? "active" : ""}`}
                  onClick={() => setActiveBlock("maquinas")}
                >
                  <Tractor size={24} />
                  <span>
                ...
                <button
                  className={`farm-overview-block ${activeBlock === "funcionarios" ? "active" : ""}`}
                  onClick={() => setActiveBlock("funcionarios")}
                >
                  <Users size={24} />
                  <span>
                ...
                <button className="farm-overview-block" type="button">
                  <Calendar size={24} />
                  <span>
              ...
              {activeBlock === "documentos" && documentsModalOpen && (
                <div className="nested-document-modal">
                  <div className="nested-document-modal-header">
                    <strong>Documentos da Fazenda</strong>
                    <div>
                      <span>{farm.checklist?.length ?? 0} checklist(s)</span>
                      <button
                        className="modal-close-btn"
                  ...
                  <fieldset className="documents-fieldset">
                    <legend>Documentos da Fazenda</legend>
                    <div className="document-search-block">
                      <Search size={17} />
                      <input
                    ...
                    <div className="farm-checklist-links legacy-document-list">
                      {farm.checklist?.length ? (
                        farm.checklist
                          .filter((item) =>
                            (item.label ?? item.documentKey)
                              .toLocaleLowerCase("pt-BR")
                              .includes(
                                documentSearch.toLocaleLowerCase("pt-BR"),
                              ),
                          )
                          .map((item) => {
                            ...
                            return (
                              <button
                                className="farm-checklist-link"
                                key={item.documentKey}
                                disabled={!folder}
                                onClick={() => {
                                  if (folder)
                                    window.location.href = `/pastas?farmId=${farm.id}&folderId=${folder.id}`;
                                }}
                              >
                                <FileText size={18} />
                                <span>{item.label ?? item.documentKey}</span>
                                <span className="farm-checklist-arrow">
                                  {folder
                                    ? "Abrir pasta →"
                                    : "Pasta indisponível"}
                                </span>
              ...
              {informationModalOpen && (
                <div className="nested-document-modal farm-information-modal">
                  <div className="nested-document-modal-header">
                    <strong>Informações da Fazenda</strong>
                    <button
                      className="modal-close-btn"
                    ...
                    <table className="farm-information-table">
                      <thead>
                        <tr>
                          ...
                          <th>COMENTARIOS DA RENOVAÇÃO</th>
...
function AgendaPage({ token }: { token: string }) {
  ...
  async function load() {
    const r = await fetch(`${API_URL}/obligations`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!r.ok) throw new Error("Não foi possível carregar a agenda.");
  ...
  async function save(event: FormEvent) {
    ...
    const r = await fetch(
      `${API_URL}/obligations${editing ? `/${editing.id}` : ""}`,
      {
    ...
    if (!r.ok) {
      setError("Não foi possível salvar a obrigação.");
      return;
    }
    setFormOpen(false);
    await load();
  ...
  async function toggle(item: Obligation) {
    const r = await fetch(`${API_URL}/obligations/${item.id}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
  ...
  async function remove(item: Obligation) {
    if (!window.confirm(`Deseja excluir a obrigação "${item.title}"?`)) return;
    const r = await fetch(`${API_URL}/obligations/${item.id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  return (
    <div id="tab-agenda" className="tab-page active">
      <div className="agenda-layout">
        ...
        <div className="tasks-panel">
          <div className="tasks-header">
            <h3>Obrigações e Lembretes Fiscais</h3>
          ...
          {error && <div className="login-error">{error}</div>}
          {formOpen && (
            <form className="add-task-form visible" onSubmit={save}>
              <div className="form-row">
                <input
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="Nome do imposto ou documento..."
                  required
                />
              </div>
              <div className="form-row">
              ...
              <div className="form-actions">
                ...
                <button className="btn btn-primary" type="submit">
                  {editing ? "Salvar Alterações" : "Salvar Obrigação"}
                </button>
              </div>
            </form>
          )}
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className="task-info">
                  ...
                  <div className="task-meta">{formatDate(item.dueDate)}</div>
                </div>
...
function CalendarGrid({ items }: { items: Obligation[] }) {
  ...
  function dayItems(day: number) {
    ...
    return items.filter((item) => item.dueDate.slice(0, 10) === key);
  }
  return (
    <>
      <div className="calendar-header">
        ...
        <div className="calendar-nav">
          <button onClick={previous} aria-label="Mês anterior">
            <ChevronLeft size={16} />
          </button>
          <button onClick={next} aria-label="Próximo mês">
      ...
      <div className="calendar-grid">
        ...
        {Array.from({ length: firstDay }).map((_, index) => (
          <div className="calendar-day other-month" key={`empty-${index}`} />
        ))}
        {Array.from({ length: days }).map((_, index) => {
          const day = index + 1;
          const dayItemsList = dayItems(day);
          return (
            <div
              className={`calendar-day ${dayItemsList.length ? "has-pending-payment" : ""}`}
              key={day}
              title={dayItemsList.map((item) => item.title).join(" | ")}
            >
              {day}
            </div>
          );
...
function FoldersPage({ token }: { token: string }) {
  return <FolderManager token={token} />;
}
