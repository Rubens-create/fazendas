...
function FarmDetailsModal({ farm, token, onClose }: { farm: Farm; token: string; onClose: () => void }) {
  ...
  async function updateChecklistField(itemId: string, field: 'documentDate' | 'dueDate' | 'renewalComments' | 'renewalDate' | 'documentStatus', value: string) {
    setEditableChecklist(items => items.map(item => item.id === itemId ? { ...item, [field]: value || null } : item));
    const previous = checklistUpdateQueues.current[itemId] ?? Promise.resolve();
    const update = previous.catch(() => undefined).then(async () => {
      const response = await fetch(`${API_URL}/farms/${farm.id}/checklist/${itemId}`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ [field]: value || null }) });
      if (!response.ok) throw new Error('Não foi possível salvar a informação do documento.');
    });
    checklistUpdateQueues.current[itemId] = update;
    try { await update; } finally { if (checklistUpdateQueues.current[itemId] === update) delete checklistUpdateQueues.current[itemId]; }
  }
